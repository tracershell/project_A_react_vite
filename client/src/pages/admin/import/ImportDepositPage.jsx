...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (item) {
     ...
     // ✅ 시 + 분 + 초 기반 랜덤 코드 생성
    const now = new Date();
    const hh = ('0' + now.getHours()).slice(-2);     // HH
    const mm = ('0' + now.getMinutes()).slice(-2);   // MM
    const ss = ('0' + now.getSeconds()).slice(-2);   // SS
    const randomCode = `${hh}${mm}${ss}`;            // "142537"

    const po_no_unique = `${item.po_no || 'EXTRA'}_${randomCode}`;
